...
export default function Sidebar({ isCollapsed, onToggle }) {
  ...
  return (
    <aside className={`fixed top-0 left-0 h-screen bg-white border-r border-[var(--borderOutline)] flex flex-col transition-all duration-300 shadow-md ${isCollapsed ? 'w-20' : 'w-64'}`} style={{zIndex: 50}}>
      <div className="p-4 border-b border-[var(--borderOutline)] bg-white sticky top-0 z-10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            {!isCollapsed && <span className="text-xl font-bold text-[var(--primaryBlue)] tracking-tight">만나식권 관리자</span>}
          </div>
          <button
            onClick={onToggle}
        ...
        <ul className="space-y-1 px-2">
          {menus.map((menu) => (
            <li key={menu.path}>
              ...
              >
                <span className={`flex items-center justify-center ${isCollapsed ? 'w-full' : ''}`}>
                ...
                {!isCollapsed && <span className="text-base font-medium whitespace-nowrap">{menu.label}</span>}
              </button>
            </li>
          ))}
        </ul>
