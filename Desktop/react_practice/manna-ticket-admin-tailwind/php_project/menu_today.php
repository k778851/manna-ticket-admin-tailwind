...
<body>
  <div class="container">
    <h1 style="font-size:2rem; font-weight:bold; margin-bottom:16px;">오늘 메뉴</h1>
    <div class="tab-menu">
      <a href="menu_today.php"><button class="tab-btn active">오늘 메뉴</button></a>
      <a href="menu_week.php"><button class="tab-btn">주간 메뉴</button></a>
    ...
    <div style="margin-bottom:16px;">
      ...
      <button class="btn btn-edit" onclick="showAlert('메뉴 복사는 실제 동작하지 않습니다.')">메뉴 복사</button>
      <button class="btn btn-edit" onclick="showAlert('알레르기 정보 수정은 실제 동작하지 않습니다.')">알레르기 수정</button>
      <button class="btn btn-edit" onclick="showAlert('영양 정보 표시는 실제 동작하지 않습니다.')">영양 정보</button>
    </div>
    <table class="menu-table">
      <tr><th>구분</th><th>날짜/종류</th><th>메인요리</th><th>반찬</th><th>국/스프</th><th>알레르기</th><th>작업</th></tr>
      <?php foreach($todayMenus as $row): ?>
      <tr>
        <td><?= htmlspecialchars($row['type']) ?></td>
        <td><?= htmlspecialchars($row['date']) ?></td>
        <td><?= htmlspecialchars($row['main']) ?></td>
        <td><?= htmlspecialchars($row['sides']) ?></td>
        <td><?= htmlspecialchars($row['soup']) ?></td>
